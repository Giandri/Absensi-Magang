...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  return (
    <div className="grid grow grid-cols-7">
      ...
      {days.map((day, index) => {
        const isWeekend = (index + startDay) % 7 === 0 || (index + startDay) % 7 === 6;
        return (
          <div
            className={cn(
            ...
            key={index}
          >
            <div className={cn("h-full w-full", isWeekend && "opacity-60")}>
              {day}
            </div>
          </div>
        );
...
export const CalendarItem = memo(
  ({ feature, className, style, children }: CalendarItemProps) => (
    <div 
